# Decisões técnicas e limitações conhecidas — PhotoCRM
...
3. **Docker Compose com serviços isolados**: `nginx` (entrada única), `web`, `api`, `db` (PostgreSQL 17), `redis` (Redis 7), `worker`. Rede interna `photocrm_internal`; `db`/`redis` sem portas públicas; healthchecks em todos os serviços.
4. **Isolamento de máquinas compartilhadas**: projeto Compose exclusivo `photocrm`; volumes `photocrm_pgdata`/`photocrm_redisdata`; única porta publicada no host = Nginx em `${PHOTOCRM_PORT:-8080}`. **Porta 3000 do host já é usada pelo projeto `firefly_telegram` desta máquina — o PhotoCRM não publica 3000/8000/5432/6379 no host.** Em conflito, muda-se a porta do PhotoCRM, nunca a do outro projeto. Proibidos prunes e `docker compose down` sem `-p photocrm -f docker/docker-compose.yml`.
