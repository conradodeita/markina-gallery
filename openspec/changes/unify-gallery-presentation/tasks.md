...
## 5. Remediação do retorno visual e proteção dissuasória

- [x] 5.1 Reorganizar Proteção visual global em controles e prévia agrupados, responsivos e legíveis; verificar alteração, salvamento, erro e contraste sem simular fotografia.
- [x] 5.2 Refinar a composição compartilhada de fotógrafo e cliente com contexto, capa, pastas, grade e visualizador editoriais; verificar que cada papel conserva somente suas ações e conteúdo autorizado.
- [x] 5.3 Implementar dissuasão de cópia nas prévias com bloqueio de arraste, menu de contexto e cópia comum, além de aviso acessível para interação protegida e `PrintScreen` detectável; verificar que o texto não promete impedir screenshots.
- [x] 5.4 Atualizar testes das duas superfícies e de Configurações; executar lint, typecheck, testes, build e validação OpenSpec estrita antes de novo deploy e revisão humana.

## Evidências da remediação visual

- Proteção visual global (2026-08-30): `npm exec vitest run app/admin/settings/page.test.tsx -- --pool=threads --maxWorkers=1` aprovou 5 testes, incluindo agrupamento de conteúdo/aparência, prova de contraste declarada como não fotográfica, alteração reativa, salvamento e erro; `npx eslint app/admin/settings/page.tsx app/admin/settings/page.test.tsx` terminou sem erros (1 aviso conhecido de `<img>`).
- Apresentação compartilhada e dissuasão (2026-08-30): `npm exec vitest run app/admin/galleries/sources/[sourceId]/preview/page.test.tsx app/gallery/gallery.test.tsx -- --pool=threads --maxWorkers=1` aprovou 8 testes. A cobertura confirma a mesma estrutura editorial para os dois papéis, ações de cliente ausentes na prévia administrativa, fotografia antes do resumo transacional, URLs protegidas, arraste desativado e mensagens acessíveis para menu de contexto e `PrintScreen`; o aviso afirma expressamente que screenshots não podem ser impedidos pelo navegador.
- Integração da remediação (2026-08-30): `npm run lint` terminou com 0 erros e 16 avisos conhecidos; `npx tsc --noEmit` foi aprovado; `npm test -- --run --pool=threads --maxWorkers=1` aprovou 47 testes em 15 arquivos; `npm run build` concluiu a compilação de produção no Next.js 16.3.2. `npx --yes @fission-ai/openspec@1.11.0 validate unify-gallery-presentation --strict`, a validação estrita da change administrativa correlata e `validate --all --strict` foram aprovados (21 itens, 0 falhas).
- Publicação da remediação (2026-08-30): PR `#12` integrado em `develop` como `bb079e7c7fb037dae5794cd820925a1c3190d51f`; o run `33313549379` e o deployment `6167002795` concluíram backend, frontend, OpenSpec, gitleaks e `deploy-homolog` com sucesso. O log do deploy confirmou Alembic em `20260829_0015 (head)` antes e depois da publicação. A verificação externa posterior obteve HTTP 200 e corpo `ok` em `/healthz`, além de HTTP 200 e `{"status":"ok","service":"api"}` em `/api/health`.
- Revisão visual publicada (2026-08-30): a entrada e os shells de fotógrafo e cliente foram abertos em homologação, inclusive com viewport de smartphone, sem contornar autenticação. A área administrativa respondeu com `Não foi possível carregar as galerias` e a biblioteca com `Biblioteca indisponível`, coerentes com a ausência de sessão OTP autenticada/dados sintéticos acessíveis ao navegador. A task 3.3 permanece pendente para uma pessoa autenticar os dois papéis, abrir a prévia administrativa e a galeria derivada sintética em desktop e smartphone e confirmar a paridade visual e o limite de autorização.

## Registro de supersession — 2026-08-31

- A task 3.3 permanece revisão visual humana da composição entregue. `improve-gallery-and-client-data-lifecycle` passa a reger o conjunto Galeria pública autenticada + privada + histórico e a herança de apresentação; o trabalho local dessa change sucessora não depende da sessão humana pendente aqui.
