...
- [x] 2.3 Implementar criação/consulta/conexão da instância Baileys dedicada, identidade remetente e material efêmero de QR/pairing sem expor API key; verificar estados aberto/conectando/fechado, número coincidente/divergente e expiração do pareamento com fake Evolution.
  - Correção validada em 2026-08-30: a identidade reconhece estritamente o JID brasileiro legado quando apenas o nono dígito posterior ao DDD está omitido; `backend/tests/test_whatsapp_delivery.py` cobre igualdade exata, equivalência nos dois sentidos, consulta administrativa, webhook e rejeição de DDD, país ou demais dígitos divergentes (26 testes passaram).
...
- [x] 6.3 Após autorização, publicar pelo fluxo protegido, confirmar SHA, migrations, healthchecks e que o canal permanece sem efeitos externos até o pareamento; não marcar concluída sem evidência do deployment.
  - Evidência em 2026-08-30: PR #13 integrado no SHA `49474d5491d89833b1b501d471c6fca98a997b45`; run `33334772284` e deployment `6171123289` concluídos com sucesso; migration `20260830_0016 (head)`; `/healthz` e `/api/health` retornaram 200; webhook público retornou 404.
  - Evidência em 2026-08-30: Evolution API/PostgreSQL/Redis saudáveis com limites previstos, volumes exclusivos e nenhuma porta publicada; API interna retornou zero instâncias, e o banco Markina retornou zero canais e zero entregas antes do pareamento.
- [x] 6.4 Solicitar ao proprietário o número próprio de homologação e a leitura de QR ou uso do pairing code somente depois da infraestrutura pronta; confirmar conexão, identidade coincidente e recuperação após reinício sem registrar material sensível.
  - Evidência em 2026-08-30: o proprietário salvou o número próprio e concluiu o pareamento humano; a falha inicial causada pela divergência não secreta `APP_ENV=staging`/`WHATSAPP_CREDENTIAL_ENV=homolog` foi corrigida para coincidência exata.
  - Evidência em 2026-08-30: a Evolution retornou conexão `open`; a representação JID brasileira legada foi reconhecida pela equivalência estrita validada; o canal ficou `ready`, sem erro e sem registrar telefone, QR, pairing code ou sessão.
  - Evidência em 2026-08-30: após reinício controlado somente de `evolution-api`, o serviço voltou `healthy`, a sessão recuperou `open`, a identidade permaneceu coincidente e o canal retornou `ready`.
- [ ] 6.5 Validar em homologação com dados sintéticos o recebimento real de OTP, login do cliente e todas as mensagens transacionais já implementadas nas direções sistema→cliente e sistema→fotógrafo; verificar IDs/estados, ausência de duplicata em falha simulada e limites de autorização.
  - Evidência parcial em 2026-08-30: cliente, galeria, pedidos e comunicações exclusivamente sintéticos foram criados para o número próprio autorizado; um OTP real terminou `accepted` em uma tentativa, com ID externo, payload cifrado apagado e desafio ainda não consumido.
  - Evidência parcial em 2026-08-30: os templates `confirmed`, `refused` e `photographer_reported` terminaram `accepted`/`sent`, cada um em uma tentativa, com uma linha de tentativa, ID externo presente e nenhum erro; nenhum telefone ou corpo foi registrado.
  - Evidência automatizada complementar: CI do run `33337416038` validou deduplicação, falha ambígua sem retry cego, destinatários autorizados, rate limits, autenticação administrativa e limpeza de OTP.
  - Bloqueio humano restante: o proprietário precisa confirmar no aparelho o recebimento e consumir um novo OTP pela própria tela de login; somente então o login real pode ser marcado como validado.
