...
## Consulta compacta do histórico administrativo

Após confirmar que a exclusão funcionou, o proprietário solicitou que os movimentos removidos não formem um log aberto na página. Vendas e pagamentos terá seletor entre a visão financeira padrão e Histórico de acervo excluído. Endpoint administrativo dedicado aplica filtros de nome do cliente, UUID histórico de galeria e período do movimento antes de LIMIT/OFFSET (25 por padrão, máximo 50), ordenado por data e UUID. Uma linha extra indica próxima página, sem carregar todo o histórico. As opções de galeria vêm dos próprios snapshots, incluindo seleção sem venda. A interface substitui páginas e recolhe detalhes por identidade do cliente e galeria, sem depender da existência do acervo. Requisições antigas são abortadas ao mudar consulta. A projeção financeira mantém pedidos/decisões e retorna lista vazia no campo legado removed_movements. O histórico do portal cliente permanece no escopo existente; esta melhoria trata da administração.
