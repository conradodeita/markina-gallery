...
O PhotoCRM começa sem repositório, sem estrutura de engenharia e sem especificações consolidadas; a INSTRUCOES_EXECUTOR_CLAUDE_CODE.md (§1.1 e §11) e o ROADMAP_ARQUITETURA.md (Fase 0) exigem que a primeira mudança estabeleça a fundação do projeto antes de qualquer implementação.
...
- Scaffolding técnico mínimo: estruturas vazias de `frontend/` (Next.js) e `backend/` (FastAPI) com endpoints `/health` reais — sem autenticação, CRUD ou regras de negócio do PhotoCRM.
...
- Nenhum código de negócio (autenticação, CRUD ou regras do PhotoCRM) e nenhum deploy nesta mudança; o único código é o scaffolding mínimo com `/health`.
