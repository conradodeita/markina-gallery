...
### Refinamento: pop-up de progresso na foto ampliada

Pedido posterior ao merge do PR #94, implementado em `feature/facial-search-progress-popup`. O toque abre imediatamente status flutuante no stage da fotografia. Admissão/fila/organização usam barra indeterminada; preparação e comparação usam seus contadores reais. Somente a consulta por toque iniciada na página alimenta esse pop-up. Novos toques faciais ficam desabilitados durante andamento; seleção/navegação/fechamento permanecem externos ao stage. Fechamento manual mantém polling; sucesso mantém fechamento automático e scroll único.

- `npm test -- app/face-region-viewer.test.tsx app/public-galleries/public-gallery.test.tsx`: **21 testes aprovados**. Incluem admissão sem percentual antigo, fases 50%/75%, estados terminais, polling a 25%, fechamento manual com continuidade e sucesso com scroll único.
- `npm test -- --maxWorkers=1`: **330 testes / 45 arquivos aprovados**, 153,06 s. Tentativa anterior com dois workers teve 261 testes aprovados e timeout ao iniciar dois processos de teste; repetição completa sequencial passou. `npm run lint`: zero erros, 27 avisos preexistentes.
- `npm run build`: aprovado, incluindo TypeScript. Primeira tentativa encontrou uso indevido de `exact` no teste Testing Library; removido antes do build aprovado.
- OpenSpec estrito completo: **57 itens aprovados**.
- Chrome/Playwright sobre build de produção com APIs e imagens sintéticas: **320×640, 390×844, 740×360 e 1440×900 aprovados**. Pop-up integralmente dentro do stage, navegação fora dele e dentro da viewport; barra inicialmente indeterminada, atualização real a 25%, persistência ao navegar para próxima foto e fechamento/scroll ao terminar. Screenshots mobile/paisagem inspecionados. Artefatos locais `.codex-tmp/facial-popup-*` fora do Git.
- Sem alterações de backend, contratos HTTP, migrações, configuração ou deploy. Manter parada após push + PR para conferência humana do CI.
