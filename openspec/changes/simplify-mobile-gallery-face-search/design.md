...
Refinamento após PR #94: exibir cartão flutuante central no stage da foto, independente da transformação de zoom/pan. O cartão é um status acessível dentro do diálogo existente, sem outro modal/foco preso. Começa com barra indeterminada durante admissão/fila; usa contadores da fase de indexação ou comparação quando disponíveis, nunca percentual simulado. Estado identificado pela consulta por região iniciada na página evita reaproveitar progresso de upload/consulta anterior. Durante andamento, impedir novo toque facial; navegação e fechamento permanecem externos ao stage. Em sucesso, aplica-se o fechamento/scroll existente; em erro/sem candidatas, remover o pop-up e manter mensagem e navegação. Fechar a foto mantém polling e painel da página.
