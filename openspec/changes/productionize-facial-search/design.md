...
Na entrada da referência, a interface móvel oferecerá duas origens explícitas. `Escolher foto do celular` acionará um campo JPEG sem atributo `capture`, preservando o seletor de arquivos/fototeca do sistema; `Usar câmera` acionará outro campo JPEG com `capture="user"`. Os campos nativos permanecerão ocultos atrás de controles Markina, e o diálogo organizará finalidade, origem, declaração e consentimento em blocos legíveis com ações de toque responsivas. A referência selecionada não será pré-visualizada, convertida, copiada para storage do navegador nem enviada antes da confirmação final.

O contrato de tamanho aceitará JPEGs de até 30 MiB (`31.457.280` bytes). A disponibilidade autenticada publicará esse limite numérico para que o navegador rejeite arquivos maiores antes da transferência, enquanto a API repetirá a validação como fronteira de confiança. O Nginx usará `client_max_body_size 32m` apenas como margem de transporte; isso evita que o proxy devolva HTML genérico antes da resposta da aplicação sem aumentar o limite funcional de 30 MiB. O deploy de homologação injetará explicitamente o limite funcional em cada publicação para não depender de eventual valor legado no arquivo de ambiente remoto, sem alterar segredos. Tipo JPEG, quantidade máxima de pixels e proteção contra descompressão permanecem obrigatórios. Em desktop, o diálogo será centralizado, terá largura limitada e rolagem interna dentro do viewport; em telas estreitas, será apresentado como folha inferior com ações empilhadas.

O limite funcional do arquivo não autoriza o detector a usar a resolução integral como tensor de trabalho. A referência continuará sendo validada e aceita em sua resolução original, mas o provider criará em memória uma cópia proporcional limitada a 1.600 pixels no maior lado e 2.000.000 de pixels antes do YuNet/SFace. A referência armazenada não será regravada e as fotos do acervo não serão alteradas. Se o processo for interrompido fora da fronteira de exceção, o lease expirado permitirá novo claim; ao exceder o mesmo máximo de tentativas das falhas capturadas, o próximo consumidor encerrará a consulta como falha sanitizada e eliminará a referência, impedindo permanência indefinida em `validating_reference`.

Na Galeria pública, a classe exclusiva da rota ampliará apenas seu conteúdo para até 1.760 pixels, preservando a largura de leitura das demais telas da cliente. A grade manterá quatro colunas no desktop e os breakpoints móveis existentes, fazendo as fotos crescerem com a área útil em vez de adicionar cards menores. A visualização ampliada poderá usar a largura e a altura úteis do viewport, sempre com `object-fit: contain`, sem recorte da fotografia.
