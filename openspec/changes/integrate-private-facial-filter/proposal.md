...
- Operação: homologação privada pode usar dados reais de adultos e menores quando o lote for enviado pelo administrador/fotógrafo, tiver origem documentada e autorização humana explícita para a execução, e permanecer sob acesso autenticado, criptografia, retenção mínima e exclusão controlada. Produção permanece bloqueada até RIPD, textos jurídicos, mecanismo de representação legal quando aplicável, calibração permitida, revisão de segurança e autorização humana de deploy.
