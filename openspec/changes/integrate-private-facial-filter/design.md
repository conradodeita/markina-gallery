...
`FACIAL_PROCESSING_ENABLED=false` será o kill switch técnico de ambiente. Quando o operador habilitar o subsistema com todas as versões, chaves e modelos válidos, o backend criará ou reconciliará automaticamente uma `GalleryFacialPolicy` interna para cada Galeria pública ativa, como snapshot técnico de versões, limiar e modelo. Esse registro não é uma política que o administrador prepara, aceita ou desbloqueia; para ele, novos jobs dependem apenas da disponibilidade técnica global e da elegibilidade da foto.

O fotógrafo não declara autorização, não prepara política e não ativa o filtro por galeria na interface. Adultos e menores do acervo percorrem a mesma indexação automática; `FACIAL_MINOR_SEARCH_ENABLED` pertence exclusivamente à posterior consulta da cliente com uma referência declarada infantil e não é requisito para o processamento administrativo. O painel é informativo e usa somente os estados `processing`, `completed` e `failed`, além de progresso, falhas sanitizadas e retentativa. A autorização operacional para dados reais continua externa ao fluxo da galeria. A homologação privada pode usar lote real de adultos e menores enviado pelo administrador/fotógrafo quando houver origem documentada, autorização humana explícita para a execução, acesso autenticado, criptografia, retenção mínima e exclusão controlada.
...
O frontend mostrará `Procurar por reconhecimento facial` como ação opcional, modal de consentimento destacado, upload mobile-first, progresso cancelável e mensagens orientativas. O fotógrafo verá indexação automática com `prontas/total`, fila, processamento, falhas e retentativa, sem botões, estados ou linguagem de preparar, ativar, suspender, revogar ou desbloquear política. O badge administrativo será somente `Reconhecimento em processamento`, `Reconhecimento concluído` ou `Reconhecimento falhou`. A cliente verá etapas e contagens reais do snapshot; não haverá percentual inventado quando uma etapa não expuser unidade mensurável.
...
O painel separará conclusão técnica de cobertura facial. `processadas/total` mede jobs concluídos; `fotos com rosto/total` consulta apenas a existência de pelo menos um embedding compatível por foto e produz a porcentagem usada pelo fotógrafo como apoio para decidir o compartilhamento. Uma foto frontal sem correspondência com cliente ainda pode conter rosto e entra na cobertura; uma foto sem face detectável termina tecnicamente com sucesso e fica fora apenas da cobertura. Não haverá bloqueio automático de publicação nem limiar inventado nesta change.
...
Para retorno após fechamento da aba ou abertura do link de notificação, a API permitirá recuperar somente a consulta mais recente, ainda válida, da combinação autenticada `Galeria pública + cliente`. O endpoint repete sessão, vínculo e escopo e não aceita cliente por parâmetro. Isso torna a notificação útil sem colocar o `request_id` na mensagem e sem depender de `sessionStorage`.

O contrato de capacidade aceita pelo menos 100 uploads de referência concorrentes como jobs duráveis distintos. “Concorrente” significa admissão simultânea e espera segura em fila, não 100 inferências paralelas no host ARM. O `face-worker` preserva o limite de recursos, enquanto cada cliente vê seu próprio estado de fila/progresso e recebe outbox terminal idempotente; seleção de candidatas continua nas transações HTTP normais, isolada por cliente e protegida pela mesma autorização.
...
Antes da mutação, a operação inventaria o host, comprova cobertura exata do lote por jobs terminais ou pendentes e recusa cancelamentos, foto sem job ou job com lease ainda vigente. Um job `processing` cuja lease expirou após interrupção controlada é parte da cobertura pendente e será reclamado pelo repositório durável normal. Até dez falhas técnicas terminais podem permanecer como cobertura: reabrir a janela não as mascara nem as reenfileira; a retentativa protegida continua sendo uma operação separada. A retomada também aceita `FACIAL_PROCESSING_ENABLED=false` quando o lote foi pausado para upgrade, mantendo o manifesto privado. Depois de aguardar o worker de mídia ficar ocioso, a operação faz backup restrito do ambiente e manifesto, atualiza atomicamente apenas o vencimento e recria `api`, `worker` e `face-worker` da Markina para que todos recebam o mesmo gate. O rollback restaura ambos os arquivos e os processos anteriores sem tocar fotos, banco, proxy ou recursos de terceiros fora do recarregamento do Nginx da própria aplicação.
...
Se a fila chegar a zero com no máximo dez falhas técnicas e todo o restante concluído, uma retentativa protegida pode mover somente esses jobs do escopo original novamente para `queued`. Ela exige a mesma janela ainda vigente, nenhum job `queued|processing`, categorias sanitizadas conhecidas e cobertura exata do lote; preserva `attempts` e `last_error_category`, não recria job, foto ou mídia e não altera nenhum campo operacional do manifesto. O worker durável normal reclama os jobs e o benchmark retomado confirma o resultado terminal.
...
Para publicar um novo SHA com um lote privado já registrado, o trailer exato `Homolog-Facial: pause-private-for-upgrade` aciona o mesmo mecanismo conservador por `pause-private`, com confirmação própria. A pausa preserva mídia, índices, jobs, manifesto e vínculos, e o deploy não reativa o processamento. Uma execução posterior de `resume-private`, com nova janela e aprovação do Environment protegido, é necessária para restaurar API, worker de mídia e face worker no SHA novo; as falhas técnicas remanescentes continuam exigindo `retry-failed-private`.
