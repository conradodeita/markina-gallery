...
- [x] 5.8 Projetar `/library` como jornadas autoritativas agrupadas por Galeria pública, com superfície principal pública, privada contextual ou contingência, seleção/cotação individual e compatibilidade temporária dos campos legados; verificar uma entrada por origem, link privado direto, conteúdo administrativo, origem removida, bloqueio e isolamento entre clientes. Validado em 2026-09-05 com seis cenários backend direcionados cobrindo biblioteca vazia, conteúdo administrativo, primeira seleção, múltiplas origens, origem removida e bloqueio, 16 testes de autenticação confirmando o destino público prioritário e a contingência privada, e suíte backend completa com `253 passed, 1 skipped`.
...
- [x] 7.7 Migrar a biblioteca da cliente para as jornadas devolvidas pelo backend, ocultar a privada automática como card concorrente, restaurar a Galeria pública como entrada cotidiana e apresentar `Revisar seleção`, `Fotos preparadas para você` e contingência privada somente nos contextos autorizados; atualizar a diretriz de UX e verificar desktop/mobile, acessibilidade e regressão de histórico. Validado em 2026-09-05 com nove testes frontend direcionados, suíte completa com `132 passed`, ESLint sem erros, typecheck, build de produção e cenários de uma única jornada, seleção restaurada, conteúdo preparado, origem removida, histórico e estado vazio.
...
- [ ] 8.7 Submeter a experiência desktop/mobile à revisão humana em homologação, registrar bugs e evidências e manter esta task aberta até aprovação explícita.
  ...
  - [x] 8.7.14 Publicar automaticamente cada foto de conteúdo quando sua prévia protegida concluir, remover a publicação manual do fluxo normal e reconciliar fotos já processadas; verificar sucesso, falha, idempotência e ausência de exposição do original.
  - [x] 8.7.15 Reforçar a reutilização da identidade administrativa pelo telefone E.164 no login contextual, sem sobrescrever nome nem duplicar `Client`/`ClientPhone`, preservando OTP como prova de posse; verificar contagens e vínculo após o ciclo real.
  - [x] 8.7.16 Tornar links públicos e privados permanentes na interface, remover regeneração/revogação cotidiana e manter rotação apenas como operação excepcional de incidente no backend; verificar que reabrir o editor devolve o mesmo endereço.
  - [x] 8.7.17 Implementar na Galeria pública seleção e desmarcação persistentes, restauração ao reabrir, rodapé com quantidade/cotação/economia/`Prosseguir` e reflexo imediato nos agregados administrativos; verificar isolamento entre membros e atualização pelo backend.
  - [x] 8.7.18 Corrigir a preparação de histórico da desvinculação no PostgreSQL quando a cliente possui pedido confirmado, eliminando `DISTINCT` incompatível com snapshots JSON sem ampliar o alvo comercial; verificar compilação SQL, ciclo real no PostgreSQL 17, preservação do histórico e cliente sem pedido.
  - [x] 8.7.19 Após autorização específica do inventário deste fix, publicar a correção PostgreSQL pelo PR/CI e Environment existentes, confirmar SHA, head `20260903_0042` e healthchecks e liberar o reteste humano da desvinculação, sem retomar operações reais automaticamente. Publicado em `5f2236e`, run `33776335838`, deployment `6247762689`; head e healthchecks confirmados em 2026-09-03.
  - [x] 8.7.20 Repetir a limpeza integral dos testes de homologação autorizada em 2026-09-03, após inventário atualizado e backup, preservando administrador, configurações e pareamento WhatsApp; verificar contagens zeradas, head, serviços e saúde externa e registrar limites de recuperação. Run `33780211487`, SHA `95f1522`, head `20260903_0042`; inventário final zerado e sessão/configurações/WhatsApp verificados na UI.
  - [x] 8.7.21 Após a limpeza, simular em homologação o ciclo sintético de galeria, pasta, JPEGs, cliente, vínculo e desvinculação; verificar início após confirmação, tempo até conclusão e atualização da interface, preservar evidências de eventual falha e não confundir limpeza com correção do bug.
  - [x] 8.7.22 Corrigir a projeção de vínculos atuais para excluir memberships `unlinked`, impedir fallback da proprietária legada quando houver tombstone explícito e preservar clientes `blocked`, cadastros globais e histórico; verificar API e ciclo lifecycle.
  - [x] 8.7.23 Tornar diálogos administrativos roláveis e com ações alcançáveis em viewport de baixa altura; verificar o componente compartilhado e a confirmação real da desvinculação em `624 × 484`.
  - [x] 8.7.24 Após validação automatizada e autorização já concedida, publicar o fix pelo PR/CI e Environment existentes sem limpeza, confirmar SHA, head `20260903_0042`, healthchecks e repetir o fluxo sintético preservado até a atualização correta da interface.
  - [x] 8.7.25 Corrigir a abertura e a exclusão explícita de privada preservada após a remoção de sua Galeria pública de origem, sem reativar a origem nem apagar cliente, mídia pública ou histórico; verificar API, componente e regressão no ciclo de lifecycle. Validado no PostgreSQL 17, suíte backend `253 passed, 1 skipped`, frontend `130 passed`, Ruff, ESLint sem erros, TypeScript, build e OpenSpec estrito em 2026-09-04.
  - [x] 8.7.26 Permitir ativar, desativar e reativar tabelas globais de preço pela área administrativa, mantendo somente as ativas no dropdown e preservando versão, faixas e snapshots; verificar API idempotente, componente e ciclo real em homologação. Validado no PostgreSQL 17, backend `253 passed, 1 skipped`, frontend `131 passed`, Ruff, ESLint sem erros, TypeScript, build e OpenSpec estrito; CI/deploy `33933710681`/`6275302590` no SHA funcional `f15581a`, migration `20260903_0042 (head)`, healthchecks externos 200 e ciclo Desativar → Ativar confirmado em homologação preservando versão 2 e três faixas em 2026-09-04.
  - [ ] 8.7.27 Após autorização e deploy, validar em homologação que uma primeira seleção cria a privada operacional sem duplicar o card da cliente, que o retorno abre a Galeria pública com seleção restaurada e que conteúdo preparado, link privado direto, origem indisponível e histórico continuam acessíveis conforme autorização.
