...
### Tasks 8.7.14 a 8.7.17 — disponibilidade automática, identidade, links e seleção pública

- O worker passa a disponibilizar a foto de conteúdo e liberar sua pasta na mesma transação que conclui todas as derivadas, somente depois de existir `client_preview` protegida. Fonte ausente ou falha de processamento não publica a foto. A migration de dados `20260903_0042` reconcilia fotos de conteúdo já processadas sem alterar capas, e seu downgrade é deliberadamente vazio porque não é possível distinguir com segurança a disponibilidade anterior.
- A etapa Imagens acompanha processamento em andamento e não possui mais publicação manual. Upload e navegação continuam imediatos; cada prévia concluída aparece automaticamente para administrador e clientes autorizadas, enquanto originais permanecem fora das respostas e URLs de apresentação.
- O login contextual resolve primeiro `ClientPhone` ativo em E.164 e reutiliza o mesmo `Client.id` cadastrado pelo administrador. O nome administrativo não é sobrescrito e não surge um segundo `Client` ou `ClientPhone`; o primeiro acesso sem sessão válida continua exigindo OTP como prova de posse do número.
- A interface expõe somente criação inicial e cópia dos links permanentes. Rotação e revogação permanecem no backend para resposta excepcional a incidente, sem ação cotidiana capaz de invalidar um endereço já compartilhado.
- A Galeria pública restaura seleções persistidas, permite selecionar e desmarcar, recebe a cotação autoritativa do backend e mostra quantidade, total, economia e `Prosseguir` no rodapé flutuante. A mesma `PhotoSelection` alimenta os agregados administrativos antes do checkout; ao desmarcar em uma privada compartilhada, referências ainda usadas por outro membro são preservadas.
- Validação dirigida: backend `6 passed` cobrindo worker, falha, migration, identidade e seleção isolada; backend ampliado `89 passed`; frontend dirigido `56 passed`. Validação integral: backend `252 passed, 1 skipped`; frontend `24` arquivos e `128 passed`; ESLint sem erros (22 avisos conhecidos), TypeScript e build Next.js `16.3.2` concluídos, com 26 rotas. Ruff dirigido, `git diff --check`, head Alembic único `20260903_0042` e OpenSpec estrito aprovados.
- O PR `#39` integrou o incremento como `d37301e0d38de8e41a720cacf3f19c5ba5fc9499`. O run `33748208935` e o deployment `6242613460` concluíram verdes; backup lógico foi criado, Alembic avançou de `0041` para `0042` e somente `api`, `web`, `worker` e `nginx` foram recriados. Às `11:14:47Z`, `/healthz` e `/api/health` responderam `200`; homologação está em paridade funcional e aguarda o reteste humano da task 8.7.

### Task 8.7.18 — preparação de histórico no PostgreSQL

- A correção anterior da task 8.7.11 alcançou a consulta de política comercial, mas não a segunda consulta de `materialize_commercial_history`, executada quando há compra confirmada. O ciclo de desvinculação falhou em PostgreSQL 17 na etapa `preparing_history`, com `could not identify an equality operator for type json`: a consulta selecionava `SaleOrder`, incluindo `price_rule_snapshot`, com `DISTINCT` e `FOR UPDATE`.
- A consulta de materialização agora usa `commercial_history_orders_query`, sem `DISTINCT`, mantendo bloqueio das linhas, ordenação por criação/ID e filtros de origem/cliente. O recorte por foto usa subconsulta de IDs de pedidos em vez de join, evitando multiplicar resultados sem ampliar o alvo comercial. Não há alteração de schema, migration, política comercial, interface ou dependência.
- Reprodução isolada: o teste `test_unlink_client_is_idempotent_scoped_and_preserves_history` falhou antes da correção e passou depois (`1 passed`) em `postgres:17-alpine`. O cenário exercita pedido confirmado com snapshot JSON, preservação do histórico e de outra origem, idempotência e desvinculação de cliente sem privada/pedido. A primeira repetição esbarrou no teardown preexistente de FKs circulares sem nome; recriar somente o banco sintético descartável permitiu a prova limpa.
- O container temporário `markina-unlink-repro-pg` e seu banco sintético foram removidos após a prova. Nenhum dado, container persistente ou credencial de homologação foi alterado, e a reprodução não constitui inspeção do registro real da cliente.
- Regressão dirigida: `python -m pytest tests/test_gallery_lifecycle.py -k "unlink_client_is_idempotent_scoped_and_preserves_history or commercial_history_lock_is_valid_postgresql_with_json_snapshot or commercial_removal_lock_is_valid_postgresql_without_distinct" -q --tb=short` aprovou `3 passed, 49 deselected`. O teste novo compila a consulta PostgreSQL com JSON e recorte por foto, assegurando presença de `FOR UPDATE` e ausência de `DISTINCT`.
- Prova adicional em PostgreSQL 17: `python -m pytest tests/test_gallery_lifecycle.py::test_commercial_removal_policy_blocks_review_cancels_pending_and_prepares_history -q --tb=short` aprovou `1 passed`, cobrindo bloqueio em análise, cancelamento de pendências elegíveis, preservação confirmada e idempotência. Sobre seus três pedidos sintéticos, uma consulta transacional com rollback aprovou 15 verificações: origem inteira, cliente, cada foto, cada foto com cliente e ausência de resultado para origem/cliente/foto divergentes. O segundo container temporário `markina-history-scope-pg`, com banco em tmpfs, também foi removido; nada foi persistido em ambiente real.
- Validação integral: `python -m pytest -q --tb=short` aprovou `253 passed, 1 skipped` em `722.76s`. O skip é o cenário opcional de migration que exige `TEST_POSTGRES_DATABASE_URL`, não a prova PostgreSQL deste fix (executada separadamente). Ruff integral (`python -m ruff check app tests`) aprovado; `python -m alembic heads` confirma `20260903_0042 (head)`. `git diff --check` sem erro e OpenSpec estrito integral com `26 passed, 0 failed`; avisos de reconciliação de arquivo de changes antigas permanecem na task 8.8. Nenhum arquivo frontend foi alterado, portanto sua suíte/build não foram repetidos neste fix backend.
- Publicação deste incremento ainda depende de autorização específica para o inventário atualizado. Uma operação anteriormente falha deverá ser retomada pela ação administrativa existente após publicação; este fix não dispara retentativas nem desvinculações automaticamente. Revisão humana 8.7 e sincronização/arquivo 8.8 permanecem pendentes.

### Task 8.7.19 — publicação autorizada do fix PostgreSQL

- Em 2026-09-03, o proprietário autorizou explicitamente push, integração e deploy do fix `9825bc9` após apresentação do inventário `e8983a8`. A base remota continua `f87a198`, o workspace estava limpo e a diferença funcional continua restrita à consulta de materialização e ao teste de regressão.
- A publicação seguirá PR com CI verde, aprovação do Environment `homolog`, backup lógico e head inalterado `20260903_0042`. Não serão retomadas desvinculações reais nem executadas limpeza, mudança de segredo ou ação em recursos de terceiros. A task somente será concluída após registrar SHA, run/deployment e healthchecks externos.
- O PR `#41` passou no run `33776049423` e foi integrado em `develop` como `5f2236ec80256ce53d5d8f1e5e5aed36c313b8b8`. O CI do SHA integrado, run `33776335838`, aprovou backend (`253 passed, 1 skipped`), frontend (`128 passed`, lint/build), OpenSpec e gitleaks antes da aprovação autorizada do Environment.
- O deployment `6247762689`, job `100719691676`, terminou `success` em `2026-09-03T16:07:41Z`. O servidor criou backup lógico, confirmou `20260903_0042 (head) -> 20260903_0042 (head)` e recriou somente API, worker e nginx. Web, PostgreSQL, Redis e Evolution permaneceram saudáveis em execução; não houve limpeza, mudança de segredo ou atuação em recursos de terceiros.
- Às `2026-09-03T16:08:58Z`, `/healthz` retornou `200`/`ok`, `/api/health` retornou `200`/`{"status":"ok","service":"api"}` e `/api/admin/parent-galleries` sem autenticação retornou `403`/`Acesso negado.`. O `develop` remoto e homologação apontavam para o mesmo SHA integrado; a documentação posterior não altera essa paridade funcional.
- Task 8.7.19 concluída. A task 8.7 continua aberta para o reteste humano em homologação, incluindo `Retomar a desvinculação` na operação anteriormente falha; nenhuma operação real foi criada ou retomada por este executor. Sincronização/arquivamento 8.8 aguarda o aceite humano. O registro pós-deploy é exclusivamente documental e não exige novo rollout.

### Tasks 8.7.22 e 8.7.23 — projeção pós-desvinculação e diálogo compacto

- O smoke posterior à limpeza provou que a operação concluía e preservava o tombstone `unlinked`, mas a API de clientes da pública o reincluía por dois caminhos: todas as memberships e `DerivedGallery.client_id` legado. A projeção atual agora usa somente memberships `active|blocked`; a proprietária legada só entra quando não existe associação explícita para a cliente naquela origem. Assim, `unlinked` encerra a superfície operacional sem apagar identidade, privada ou histórico, enquanto `blocked` permanece visível e reversível.
- O diálogo compartilhado limita sua altura à viewport e permite rolagem vertical no backdrop e no próprio painel. As consequências e ações deixam de ficar cortadas em janelas baixas; a confirmação em `624 × 484` permanece pendente da publicação autorizada em homologação.
- Regressão antes/depois: o teste lifecycle reproduziu a proprietária tombstone ainda presente e passou após a correção, incluindo cliente apenas pública e lista vazia após as duas desvinculações. O agregado comercial cobre membro `blocked` sem perder a compatibilidade de proprietárias legadas.
- Validação dirigida: os dois testes backend passaram (`2 passed`); os módulos completos `test_gallery_lifecycle.py` e `test_derived_galleries.py` passaram (`110 passed`); Ruff dirigido passou. A suíte backend integral aprovou `253 passed, 1 skipped`; o skip é a prova opcional que requer `TEST_POSTGRES_DATABASE_URL`. O frontend dirigido aprovou `46` testes e a suíte completa aprovou `129`; ESLint não teve erros (22 avisos preexistentes), `npx tsc --noEmit` e build Next.js `16.3.2` passaram com 26 rotas. Head Alembic único `20260903_0042`; validação OpenSpec estrita e política estrutural de deploy aprovadas. O teste shell de deploy não roda neste host Windows sem `/bin/bash` e permanece coberto pelo runner Linux do CI.
- A task 8.7.22 está concluída. Após o deploy autorizado, a etapa Clientes e o resumo da pública sintética exibiram zero vínculos, mantendo as duas identidades no cadastro global. Em viewport `624 × 484`, a confirmação real ficou rolável e os dois botões permaneceram alcançáveis; a operação foi cancelada sem exclusão. As tasks 8.7.21, 8.7.23 e 8.7.24 também estão concluídas; a revisão humana geral permanece aberta e nenhuma limpeza foi repetida.
