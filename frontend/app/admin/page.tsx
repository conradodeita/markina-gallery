...
export default function AdminPage() {
  ...
  return <main className="admin-shell"><ValidationHeader role="Painel do fotógrafo" version={`${summary.environment} · ${summary.version}`} /><h1>Valide seu fluxo de trabalho</h1><p className="intro">Relate nesta conversa a tela e a versão exibida quando encontrar algo inesperado.</p><section className="state-grid"><StateCard title="Clientes" value={summary.counts.clients} detail="cadastros autorizados" href="/admin/operations" /><StateCard title="Acervos" value={summary.counts.parent_galleries} detail="somente administrativos" href="/admin/operations" /><StateCard title="Galerias privadas" value={summary.counts.derived_galleries} detail="acessos derivados" href="/admin/galleries" /><StateCard title="Importações" value={processing} detail="em processamento" href="/admin/operations" /></section><section className="admin-card"><h2>Próximas validações</h2><div className="action-grid"><Link className="primary" href="/admin/galleries">Gerenciar galerias</Link><Link className="secondary" href="/admin/operations">Criar cliente e galeria</Link><Link className="secondary" href="/admin/statistics">Estatísticas</Link><Link className="secondary" href="/admin/purchases">Compras</Link></div></section><section className="admin-card"><h2>Galerias recentes</h2>{summary.recent_galleries.length ? <div className="recent-list">{summary.recent_galleries.map((gallery) => <article key={gallery.id}><strong>{gallery.name}</strong><span>{gallery.access_enabled ? "Acesso ativo" : "Acesso bloqueado"}</span></article>)}</div> : <EmptyState title="Nenhuma galeria privada ainda" detail="Comece criando cliente, acervo e JPEG na operação." />}</section></main>;
}
