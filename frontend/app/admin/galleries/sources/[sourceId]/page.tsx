...
export default function SourceGalleryDetailPage() {
  ...
  return (
    <main className="admin-shell">
      ...
      <div className="action-grid"><Link className="mk-button mk-button--primary" href={`/admin/galleries/sources/${sourceId}/preview`}>Visualizar galeria</Link><Link className="mk-button mk-button--secondary" href={`/admin/galleries/sources/${sourceId}/edit/ajustes`}>Editar galeria</Link><Link className="mk-button mk-button--secondary" href={`/admin/galleries/sources/${sourceId}/edit/imagens`}>Abrir imagens</Link><button type="button" className="mk-button mk-button--danger" onClick={deleteGallery}>Excluir galeria vazia</button></div>
      <section className="admin-card"><div className="source-summary"><Link className="source-summary-cover" href={`/admin/galleries/sources/${sourceId}/preview`}>{summary.cover_preview_url ? <img src={`/api${summary.cover_preview_url}`} alt="Capa protegida da galeria" /> : "Sem capa definida"}</Link><div><h2>Resumo da galeria</h2><p>{summary.counts.folders} pastas · {summary.counts.photos} fotos · {summary.counts.clients} responsáveis vinculados</p><label>Link para compartilhar<input ref={linkInput} readOnly value={summary.unlisted_link} /></label><button type="button" className="secondary" onClick={copyUnlistedLink}>{copied ? "Link copiado" : "Copiar link"}</button></div></div></section>
