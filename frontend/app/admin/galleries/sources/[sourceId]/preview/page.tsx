...
export default function AdminGalleryPreviewPage() {
  ...
  const presentationFolders: GalleryPresentationFolder[] = folders.map((folder) => ({ id: folder.id, name: folder.name, photos: folder.photos.filter((photo) => photo.preview_url).map((photo) => ({ id: photo.id, name: photo.name, previewUrl: `/api${photo.preview_url}` })) }));
  return <main className="admin-shell gallery-preview-page"><Link href={`/admin/galleries/sources/${sourceId}`}>← Resumo da galeria</Link><GalleryPresentation galleryName={name} coverUrl={cover ? `/api${cover}` : null} folders={presentationFolders} titleStyle={{ color: titleStyle.color, fontSize: titleStyle.fontSize, fontFamily: titleStyle.fontFamily, position: titleStyle.position }} modeLabel={<><strong>Modo fotógrafo</strong><span>Prévia administrativa com prévias protegidas; a cliente vê somente o conteúdo liberado após autenticação.</span></>} emptyDetail="Crie uma pasta e conclua o processamento das fotos para revisar a apresentação da galeria." /></main>;
}
