...
export default function AdminGalleryPreviewPage() {
  ...
  return <main className="admin-shell gallery-preview-page"><Link href={`/admin/galleries/sources/${sourceId}`}>← Resumo da galeria</Link><div className="admin-preview-warning" role="status"><strong>Modo fotógrafo</strong><span>Esta é uma visualização administrativa com prévias protegidas. O cliente verá apenas o conteúdo liberado após autenticação.</span></div><section className="gallery-preview-hero">{cover ? <img src={`/api${cover}`} alt={`Capa de ${name}`} /> : <div>Sem capa definida</div>}<h1 className={positionClass} style={{ color: titleStyle.color, fontSize: `${titleStyle.fontSize}px`, fontFamily: titleStyle.fontFamily }}>{name}</h1></section>{mode === "sequential" ? folders.map((folder) => <section className="gallery-preview-folder" key={folder.id}><h2>{folder.name}</h2><div className="folder-photo-grid">{folder.photos.map((photo) => photo.preview_url ? <figure key={photo.id}><img src={`/api${photo.preview_url}`} alt={photo.name} /><figcaption>{photo.name}</figcaption></figure> : null)}</div></section>) : <div className="gallery-preview-folder-grid">{folders.map((folder) => <section className="gallery-preview-folder" key={folder.id}><h2>{folder.name}</h2><p>{folder.photo_count} foto(s)</p>{folder.preview_url ? <img src={`/api${folder.preview_url}`} alt={`Prévia da pasta ${folder.name}`} /> : null}</section>)}</div>}</main>;
}
