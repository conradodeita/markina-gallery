...
type VisualPreview = { watermark_text: string; watermark_font: string; watermark_color: string; watermark_size: number; watermark_direction: string; folder_display_mode: string; cover_title_font: string; cover_title_color: string; cover_title_size: number; cover_title_position: string };
...
export default function GalleryEditor({ sourceId, step }: { sourceId: string; step: string }) {
  ...
  useEffect(() => {
    ...
    Promise.all([editorRequest, sectionRequest])
      .then(([editorData, sectionData]) => {
        ...
        setVisualPreview({ watermark_text: visual.watermark_text ?? "", watermark_font: visual.watermark_font ?? "sans-serif", watermark_color: visual.watermark_color ?? "#FFFFFF", watermark_size: visual.watermark_size ?? 32, watermark_direction: visual.watermark_direction ?? "diagonal", folder_display_mode: visual.folder_display_mode ?? "individual", cover_title_font: visual.cover_title_font ?? "sans-serif", cover_title_color: visual.cover_title_color ?? "#FFFFFF", cover_title_size: visual.cover_title_size ?? 32, cover_title_position: visual.cover_title_position ?? "bottom-left" });
  ...
  function updateVisualPreview(event: ChangeEvent<HTMLFormElement>) {
    const form = new FormData(event.currentTarget);
    setVisualPreview({ watermark_text: String(form.get("watermark_text") ?? ""), watermark_font: String(form.get("watermark_font") ?? "sans-serif"), watermark_color: String(form.get("watermark_color") ?? "#FFFFFF"), watermark_size: Number(form.get("watermark_size") ?? 32), watermark_direction: String(form.get("watermark_direction") ?? "diagonal"), folder_display_mode: String(form.get("folder_display_mode") ?? "individual"), cover_title_font: String(form.get("cover_title_font") ?? "sans-serif"), cover_title_color: String(form.get("cover_title_color") ?? "#FFFFFF"), cover_title_size: Number(form.get("cover_title_size") ?? 32), cover_title_position: String(form.get("cover_title_position") ?? "bottom-left") });
  }
    ...
    <main className="admin-shell gallery-editor-shell">
      ...
      {currentStep === "imagens" ? (
        <section className="gallery-editor-panel">
          ...
          <form className="gallery-settings-form gallery-visual-settings" onSubmit={saveVisualSettings} onChange={updateVisualPreview}><div className="section-heading"><div><p className="eyebrow">Personalização</p><h3>Uma aparência coerente, sem editor livre</h3><p className="gallery-scope-note">Os ajustes abaixo usam apenas opções aprovadas e são aplicados às prévias protegidas.</p></div></div><div className="gallery-customization-layout"><div className="gallery-customization-panels"><fieldset className="gallery-customization-panel"><legend>Marca-d’água</legend><p>Protege cada prévia servida pela galeria.</p><label>Texto da marca-d’água<input name="watermark_text" defaultValue={editor.gallery.watermark_text} /></label><label>Tipografia da marca-d’água<select name="watermark_font" defaultValue={editor.gallery.watermark_font}><option value="sans-serif">Sans-serif</option><option value="serif">Serifada</option><option value="monospace">Monoespaçada</option><option value="DejaVuSans">DejaVu Sans</option><option value="DejaVuSerif">DejaVu Serif</option></select></label><label>Cor da marca-d’água<input name="watermark_color" type="color" defaultValue={editor.gallery.watermark_color} /></label><label>Tamanho da marca-d’água<input name="watermark_size" type="number" min={10} max={96} defaultValue={editor.gallery.watermark_size} /></label><label>Direção<select name="watermark_direction" defaultValue={editor.gallery.watermark_direction}><option value="horizontal">Horizontal</option><option value="vertical">Vertical</option><option value="diagonal">Diagonal</option></select></label></fieldset><fieldset className="gallery-customization-panel"><legend>Capa e título</legend><p>Define como o nome da galeria aparece sobre a capa protegida.</p><label>Tipografia do título<select name="cover_title_font" defaultValue={editor.gallery.cover_title_font}><option value="sans-serif">Sans-serif</option><option value="serif">Serifada</option><option value="monospace">Monoespaçada</option><option value="DejaVuSans">DejaVu Sans</option><option value="DejaVuSerif">DejaVu Serif</option></select></label><label>Cor do título<input name="cover_title_color" type="color" defaultValue={editor.gallery.cover_title_color} /></label><label>Tamanho do título<input name="cover_title_size" type="number" min={12} max={96} defaultValue={editor.gallery.cover_title_size} /></label><label>Posição do título<select name="cover_title_position" defaultValue={editor.gallery.cover_title_position}><option value="top-left">Superior esquerdo</option><option value="top-center">Superior centro</option><option value="top-right">Superior direito</option><option value="middle-left">Centro esquerdo</option><option value="middle-center">Centro</option><option value="middle-right">Centro direito</option><option value="bottom-left">Inferior esquerdo</option><option value="bottom-center">Inferior centro</option><option value="bottom-right">Inferior direito</option></select></label></fieldset><fieldset className="gallery-customization-panel"><legend>Organização</legend><p>Escolha uma estrutura de leitura suportada para as pastas da galeria.</p><label>Exibição das pastas<select name="folder_display_mode" defaultValue={editor.gallery.folder_display_mode}><option value="individual">Pastas lado a lado</option><option value="sequential">Sequência cronológica</option></select></label></fieldset></div><aside className="gallery-customization-preview" aria-live="polite"><p className="eyebrow">Prévia protegida</p>{editor.gallery.cover_preview_url ? <div className="gallery-customization-preview-image"><img src={`/api${editor.gallery.cover_preview_url}`} alt="Prévia protegida da capa da galeria" /><span className={`watermark-preview watermark-preview--${visualPreview?.watermark_direction ?? "diagonal"}`} style={{ color: visualPreview?.watermark_color, fontFamily: visualPreview?.watermark_font, fontSize: `${Math.min(visualPreview?.watermark_size ?? 20, 32)}px` }}>{visualPreview?.watermark_text || "Marca-d’água"}</span><strong className={`title-${visualPreview?.cover_title_position ?? "bottom-left"}`} style={{ color: visualPreview?.cover_title_color, fontFamily: visualPreview?.cover_title_font, fontSize: `${Math.min(visualPreview?.cover_title_size ?? 24, 34)}px` }}>{editor.gallery.name}</strong></div> : <div className="gallery-customization-preview-empty"><strong>Prévia disponível após definir uma capa</strong><span>Carregue e processe uma foto, depois escolha-a como capa para conferir a composição protegida.</span></div>}</aside></div><MarkinaButton>Salvar personalização</MarkinaButton></form>
