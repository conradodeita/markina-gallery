    ...
    <main className="admin-shell gallery-editor-shell">
      ...
      {currentStep === "imagens" ? (
        <section className="gallery-editor-panel">
          ...
          <form className="gallery-settings-form gallery-visual-settings" onSubmit={saveVisualSettings} onChange={updateVisualPreview}><div className="section-heading"><div><p className="eyebrow">Apresentação da galeria</p><h3>Capa, título e organização</h3><p className="gallery-scope-note">A marca-d’água é global e pode ser ajustada em Configurações. Aqui ficam somente as escolhas próprias desta galeria.</p></div></div><div className="gallery-customization-layout"><div className="gallery-customization-panels"><fieldset className="gallery-customization-panel"><legend>Capa e título</legend><p>Define como o nome da galeria aparece sobre a capa protegida.</p><label>Tipografia do título<select name="cover_title_font" defaultValue={editor.gallery.cover_title_font}><option value="sans-serif">Sans-serif</option><option value="serif">Serifada</option><option value="monospace">Monoespaçada</option><option value="DejaVuSans">DejaVu Sans</option><option value="DejaVuSerif">DejaVu Serif</option></select></label><label>Cor do título<input name="cover_title_color" type="color" defaultValue={editor.gallery.cover_title_color} /></label><label>Tamanho do título<input name="cover_title_size" type="number" min={12} max={96} defaultValue={editor.gallery.cover_title_size} /></label><label>Posição do título<select name="cover_title_position" defaultValue={editor.gallery.cover_title_position}><option value="top-left">Superior esquerdo</option><option value="top-center">Superior centro</option><option value="top-right">Superior direito</option><option value="middle-left">Centro esquerdo</option><option value="middle-center">Centro</option><option value="middle-right">Centro direito</option><option value="bottom-left">Inferior esquerdo</option><option value="bottom-center">Inferior centro</option><option value="bottom-right">Inferior direito</option></select></label></fieldset><fieldset className="gallery-customization-panel"><legend>Organização</legend><p>Escolha uma estrutura de leitura suportada para as pastas da galeria.</p><label>Exibição das pastas<select name="folder_display_mode" defaultValue={editor.gallery.folder_display_mode}><option value="individual">Pastas lado a lado</option><option value="sequential">Sequência cronológica</option></select></label></fieldset></div><aside className="gallery-customization-preview" aria-live="polite"><p className="eyebrow">Prévia protegida</p>{editor.gallery.cover_preview_url ? <div className="gallery-customization-preview-image"><img src={`/api${editor.gallery.cover_preview_url}`} alt="Prévia protegida da capa da galeria" /><strong className={`title-${visualPreview?.cover_title_position ?? "bottom-left"}`} style={{ color: visualPreview?.cover_title_color, fontFamily: visualPreview?.cover_title_font, fontSize: `${Math.min(visualPreview?.cover_title_size ?? 24, 34)}px` }}>{editor.gallery.name}</strong></div> : <div className="gallery-customization-preview-empty"><strong>Prévia disponível após definir uma capa</strong><span>Carregue e processe uma foto, depois escolha-a como capa para conferir a composição protegida.</span></div>}</aside></div><MarkinaButton>Salvar apresentação</MarkinaButton></form>
