...
type SourceGallery = {
  ...
  event_name: string;
  cover_preview_url: string | null;
  private_gallery_count: number;
  registration_count: number;
  frozen_gallery_count: number;
};
...
export default function GalleriesPage() {
  ...
  useEffect(() => {
    const path =
      view === "sources"
        ? "/api/admin/parent-galleries/overview"
  ...
  return (
    <main className="admin-shell">
      <div className="section-heading gallery-list-heading">
        <div>
          ...
          <p className="intro">
            ...
            uma galeria privada e um histórico independente.
          </p>
        </div>
        <Link
      ...
      {view === "private" ? (
        <>
          <div
            ...
            role="tablist"
            aria-label="Estado da galeria privada"
          >
            <button
              role="tab"
          ...
          <label className="gallery-search">
            ...
            >
              ...
              <option value="selection_finalized">Seleção finalizada</option>
      ...
      {!loading && !failed && empty ? (
        <p className="notice">Nenhum resultado nesta visão.</p>
      ) : null}
      {!loading && !failed && !empty && view === "sources" ? (
        <section className="gallery-admin-list" aria-label="Galerias do evento">
          {sources.map((source) => (
            <Link
              key={source.id}
              href={`/admin/galleries/sources/${source.id}`}
            >
              <div className="gallery-cover">
                {source.cover_preview_url ? (
                  <img src={`/api${source.cover_preview_url}`} alt="" />
                ) : (
                  "Sem capa"
                )}
              </div>
              <div>
                ...
                <span>
                  {source.registration_count} pessoas registradas ·{" "}
                  {source.private_gallery_count} galerias privadas ·{" "}
                  {source.frozen_gallery_count} congeladas
                </span>
              </div>
            </Link>
          ))}
      ...
      {!loading && !failed && !empty && view === "private" ? (
        <section className="gallery-admin-list" aria-label="Galerias privadas">
          {privateGalleries.map((gallery) => (
            <Link key={gallery.id} href={`/admin/galleries/${gallery.id}`}>
              <div className="gallery-cover">
                {gallery.cover_preview_url ? (
                  <img src={`/api${gallery.cover_preview_url}`} alt="" />
                ) : (
                  "Sem capa"
                )}
              </div>
              <div>
                ...
                <small>1 cliente · histórico independente</small>
                <span>
                  {gallery.frozen
                    ? "Prazo expirado"
                    : gallery.blocked
                      ? "Acesso bloqueado"
                      : gallery.payment_pending
                        ? "Pagamento pendente"
                        : gallery.selection_in_progress
                          ? "Seleção em andamento"
                          : "Disponível"}
                </span>
              </div>
