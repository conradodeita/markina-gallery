...
export default function GalleryDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const [photoData, memberData, availableData] = await Promise.all([
        ...
        detailData.origin_active !== false
  ...
  const originActive = detail.origin_active !== false;
  ...
  return <main className="admin-shell private-gallery-detail">
    ...
    <div className="action-grid"><MarkinaButton variant="secondary" disabled={busy || !originActive} onClick={toggle}>{detail.blocked ? "Liberar acesso geral" : "Bloquear acesso geral"}</MarkinaButton>{originActive ? <Link className="mk-button mk-button--secondary" href={`/admin/galleries/sources/${detail.parent_gallery_id}/edit/vendas`}>Preço e regras herdadas</Link> : null}<Link className="mk-button mk-button--secondary" href={`/admin/galleries/${galleryId}/orders`}>Pedidos</Link><MarkinaButton className="mk-button--danger" disabled={busy} onClick={() => { setDeleteConfirmOpen(true); setDeleteError(""); }}>Excluir galeria privada</MarkinaButton></div>
    {!originActive ? <SystemState title="Galeria pública de origem removida" detail="Esta privada e seu histórico continuam disponíveis para consulta. Não é possível adicionar fotos ou alterar regras herdadas, mas a exclusão explícita permanece disponível." /> : null}
    ...
    <section className="admin-card"><div className="section-heading"><div><h2>Pastas e fotos disponíveis</h2><p className="gallery-scope-note">Remover daqui nunca apaga o JPEG original da Galeria pública e respeita outras justificativas ativas.</p></div>{originActive ? <Link className="mk-button mk-button--secondary" href={`/admin/galleries/sources/${detail.parent_gallery_id}/edit/imagens`}>Carregar novos JPEGs na pública</Link> : null}</div>{folders.length ? <div className="private-folder-list">{folders.map((folder) => <article key={folder.id}><header><strong>{folder.name}</strong><StatusBadge>{folder.photos.length} foto(s)</StatusBadge></header><div className="private-photo-grid">{folder.photos.map((photo) => <figure key={photo.id}><img src={`/api${photo.preview_url}`} alt={`Prévia protegida de ${photo.name}`} /><figcaption><strong>{photo.name}</strong><small>{photo.origins.includes("client") ? "Mantida por seleção" : "Incluída pelo fotógrafo"}</small><MarkinaButton type="button" variant="quiet" disabled={busy || !originActive || !photo.origins.includes("admin")} onClick={() => removePhoto(photo)}>{photo.origins.includes("admin") ? "Remover inclusão" : "Seleção de cliente"}</MarkinaButton></figcaption></figure>)}</div></article>)}</div> : <SystemState title="Galeria privada sem fotos" detail={originActive ? "Adicione fotos publicadas da origem no bloco abaixo." : "A origem foi removida e não há fotos preservadas nesta privada."} />}</section>

    {originActive ? <section className="admin-card"><div className="section-heading"><div><h2>Adicionar fotos da Galeria pública</h2><p className="gallery-scope-note">A ação cria apenas referências no acervo comum e não marca fotos como selecionadas.</p></div></div>{addablePhotos.length ? <form className="private-photo-add" onSubmit={addPhotos}><div>{addablePhotos.map((photo) => <label key={photo.id}><input type="checkbox" checked={selectedPhotoIds.includes(photo.id)} onChange={(event) => setSelectedPhotoIds((current) => event.target.checked ? [...current, photo.id] : current.filter((id) => id !== photo.id))} />{photo.preview_url ? <img src={`/api${photo.preview_url}`} alt="" /> : <span>Sem prévia</span>}<strong>{photo.name}</strong><small>{photo.folder_name}</small></label>)}</div><MarkinaButton disabled={busy || !selectedPhotoIds.length}>{busy ? "Adicionando…" : "Adicionar ao acervo privado"}</MarkinaButton></form> : <SystemState title="Todas as fotos já estão disponíveis" detail="Carregue e publique novos JPEGs na Galeria pública para ampliar este acervo." />}</section> : null}

    <section className="admin-card"><h2>Ajustes da galeria privada</h2><form className="auth-form" onSubmit={saveName}><label>Nome<input name="name" defaultValue={detail.name} required /></label><MarkinaButton disabled={busy}>Salvar nome</MarkinaButton></form><p>Mensagem, prazo, favoritos, comentários e preço são herdados da Galeria pública. O PIX é global e pode ser alterado em <Link href="/admin/settings#pix">Configurações</Link>.</p>{detail.custom_message ? <p><strong>Mensagem vigente:</strong> {detail.custom_message}</p> : null}</section>
