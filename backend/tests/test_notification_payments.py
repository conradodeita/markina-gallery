...
def test_payment_decisions_unique_and_correction_silent(monkeypatch):
    ...
    assert browser.post(f"{url}/decision", json={"decision": "confirmed"}).status_code == 200
    assert browser.post(f"{url}/decision", json={"decision": "confirmed"}).status_code == 200
    with SessionLocal() as db:
        assert db.scalar(select(func.count(NotificationEvent.id))) == 2
        assert db.get(SaleOrder, order.id).payment_status == "confirmed"
    ...
    assert browser.post(f"{url}/decision", json={"decision": "refused"}).status_code == 200
    with SessionLocal() as db:
        ...
        assert db.get(PaymentCommunication, UUID(str(communication_id))).status == "refused"
        assert db.get(SaleOrder, order.id).payment_status == "cancelled"
        assert db.scalar(select(func.count(PaymentNotificationOutbox.id))) >= 2
        save_setting(db, "payment_refused", {"whatsapp_enabled": False})
        db.commit()
        projection = db.scalar(select(PaymentNotificationOutbox).where(PaymentNotificationOutbox.template_kind == "refused"))
